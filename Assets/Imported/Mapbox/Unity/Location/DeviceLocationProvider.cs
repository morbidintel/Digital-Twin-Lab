namespace Mapbox.Unity.Location
{
	...
	public class DeviceLocationProvider : AbstractLocationProvider
	{
		...
		protected virtual void Awake()
		{
			...
			currentLocation.Provider = "unity";
		...
		IEnumerator PollLocationRoutine()
		{
			...
			while (true)
			{
				...
				currentLocation.IsLocationServiceEnabled =
				...
				currentLocation.DeviceOrientation = (float)_deviceOrientationSmoothing.Calculate();
				...
				Vector2d previousLocation = new Vector2d(currentLocation.LatitudeLongitude.x, currentLocation.LatitudeLongitude.y);
				currentLocation.LatitudeLongitude = new Vector2d(latitude, longitude);

				currentLocation.Accuracy = (float)Math.Floor(lastData.horizontalAccuracy);
				...
				currentLocation.IsLocationUpdated = timestamp > _lastLocationTimestamp || !currentLocation.LatitudeLongitude.Equals(previousLocation);
				...
				if (currentLocation.IsLocationUpdated)
				{
					if (_lastPositions.Count > 0)
					{
						...
						CheapRuler cheapRuler = new CheapRuler(currentLocation.LatitudeLongitude.x, CheapRulerUnits.Meters);
						Vector2d p = currentLocation.LatitudeLongitude;
						...
						if (distance > 1.0)
						{
							_lastPositions.Add(currentLocation.LatitudeLongitude);
						}
					}
					else
					{
				...
				{
					currentLocation.UserHeading = currentLocation.DeviceOrientation;
				...
				else
				{
					...
					if (distance >= _minDistanceOldestNewestPosition)
					{
						...
						currentLocation.UserHeading = finalHeading;
						currentLocation.IsUserHeadingUpdated = true;
					}
				}
