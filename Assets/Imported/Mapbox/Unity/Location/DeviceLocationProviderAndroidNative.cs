namespace Mapbox.Unity.Location
{
	...
	public class DeviceLocationProviderAndroidNative : AbstractLocationProvider, IDisposable
	{
		...
		protected virtual void Awake()
		{
			...
			currentLocation.IsLocationServiceEnabled = false;
			currentLocation.IsLocationServiceInitializing = true;
		...
		private IEnumerator locationRoutine()
		{
			...
			while (true)
			{
				...
				if (null != _sensorInstance)
				{
					currentLocation.DeviceOrientation = _sensorInstance.Call<float>("getOrientation");
				}

				bool locationServiceAvailable = _gpsInstance.Call<bool>("getIsLocationServiceAvailable");
				currentLocation.IsLocationServiceEnabled = locationServiceAvailable;
				...
				if (!locationServiceAvailable)
				{
					...
					currentLocation.HasGpsFix = false;
					currentLocation.SatellitesInView = 0;
					currentLocation.SatellitesUsed = 0;
				...
				currentLocation.IsLocationServiceInitializing = false;
				...
				try
				{
					...
					currentLocation.TimestampDevice = UnixTimestampUtils.To(DateTime.UtcNow);
					SendLocation(currentLocation);
				}
				catch (Exception ex)
				{
		...
		private void populateCurrentLocation(AndroidJavaObject location)
		{
			if (null == location)
			{
				currentLocation.IsLocationUpdated = false;
				currentLocation.IsUserHeadingUpdated = false;
				return;
			}
			...
			bool coordinatesUpdated = !newLatLng.Equals(currentLocation.LatitudeLongitude);
			currentLocation.LatitudeLongitude = newLatLng;

			float newAccuracy = location.Call<float>("getAccuracy");
			bool accuracyUpdated = newAccuracy != currentLocation.Accuracy;
			currentLocation.Accuracy = newAccuracy;

			// divide by 1000. Android returns milliseconds, we work with seconds
			long newTimestamp = location.Call<long>("getTime") / 1000;
			bool timestampUpdated = newTimestamp != currentLocation.Timestamp;
			currentLocation.Timestamp = newTimestamp;

			string newProvider = location.Call<string>("getProvider");
			bool providerUpdated = newProvider != currentLocation.Provider;
			currentLocation.Provider = newProvider;
			...
			else
			{
				...
				currentLocation.IsUserHeadingUpdated = newHeading != currentLocation.UserHeading;
				currentLocation.UserHeading = newHeading;
			}

			float? newSpeed = location.Call<float>("getSpeed");
			bool speedUpdated = newSpeed != currentLocation.SpeedMetersPerSecond;
			currentLocation.SpeedMetersPerSecond = newSpeed;
			...
			currentLocation.IsLocationUpdated =
				providerUpdated
				|| coordinatesUpdated
				|| accuracyUpdated
				|| timestampUpdated
			...
			if (!gpsEnabled)
			{
				currentLocation.HasGpsFix = null;
				currentLocation.SatellitesInView = null;
				currentLocation.SatellitesUsed = null;
			}
			else
			{
				currentLocation.HasGpsFix = _gpsInstance.Get<bool>("hasGpsFix");
				currentLocation.SatellitesInView = _gpsInstance.Get<int>("satellitesInView");
				currentLocation.SatellitesUsed = _gpsInstance.Get<int>("satellitesUsedInFix");
			}
